...
uint32_t glfwTranslateKey(uint32_t key) {
  switch(key) {
...
auto onKeypressed = [](GLFWwindow* w, int key,int,int action,int) {
  auto translated_key = glfwTranslateKey(key);
  if(!translated_key) {
    fprintf(stderr,"invalid key: %d\n", key);
    return;
  }
  if(action == GLFW_PRESS) {
    on_keydown(translated_key);
  } else if(action != GLFW_REPEAT) {
    on_keyup(translated_key);
  }
};
...
void app_start(App_Config *conf) {
  ...
  GLFWwindow* w = glfwCreateWindow(width, height, "Last Train Home", nullptr, nullptr);
  if(!w) {
  ...
  last_time = app_time();
  double now;
  glfwSetKeyCallback(w, onKeypressed);
  while(!quit_flag) {
    reset_input();
    glfwPollEvents();
